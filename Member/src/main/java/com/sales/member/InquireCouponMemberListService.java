 ...
 */
@ServiceConfig(
        act = ActionNames.inquireCouponMemberList,
        ...
        description = "查询CouponMember配置",
        validateParameters = {
    @Field(fieldName = "session", fieldType = FieldTypeEnum.CHAR1024, description = "session信息"),
    @Field(fieldName = "encryptType", fieldType = FieldTypeEnum.TYINT, description = "加密类型")
})
public class InquireCouponMemberListService implements Service {

    private Logger logger = LogFactory.getInstance().getLogger(InquireCouponMemberListService.class);
    ...
    public void execute() {
        ...
        List<CouponMember> entityList = entityDAO.inquireByCondition(conditionList);
        if (entityList != null) {
