 ...
 */
@ServiceConfig(
        act = ActionNames.inquireReservationMemberList,
        ...
        description = "查询ReservationMember配置",
        validateParameters = {
    @Field(fieldName = "session", fieldType = FieldTypeEnum.CHAR1024, description = "session信息"),
    @Field(fieldName = "encryptType", fieldType = FieldTypeEnum.TYINT, description = "加密类型")
})
public class InquireReservationMemberListService implements Service {

    private Logger logger = LogFactory.getInstance().getLogger(InquireReservationMemberListService.class);
    ...
    public void execute() {
        ...
        List<ReservationMember> entityList = entityDAO.inquireByCondition(conditionList);
        if (entityList != null) {
