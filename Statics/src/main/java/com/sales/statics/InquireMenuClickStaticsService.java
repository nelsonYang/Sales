 ...
 */
@ServiceConfig(
        act = SalesActionName.inquireMenuClick,
        ...
        returnParameters = {"date", "clickCount"},
        description = "查询菜单点击统计列表",
        validateParameters = {
    @Field(fieldName = "session", fieldType = FieldTypeEnum.CHAR1024, description = "session类型"),
    @Field(fieldName = "encryptType", fieldType = FieldTypeEnum.TYINT, description = "加密类型")
})
public class InquireMenuClickStaticsService implements Service {
    ...
    public void execute() {
        ...
        sqlBuilder.append("select date_format( createTime,'%Y-%m-%d')  date, count(*) totalCount from weixinMessage   where companyId = ?").append(" and date_format( createTime,'%Y-%m') ='").append(currentMonth).append("'").append(" and type =7 ").append("group by date");
        ...
        for (Map<String, String> tempResultMap : resultMapList) {
            ...
            resultMap.put("clickCount", tempResultMap.get("totalCount"));
