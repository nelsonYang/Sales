 ...
 */
@ServiceConfig(
        act = ActionNames.inquireMemberCardLevelList,
        ...
        description = "查询MemberCardLevel配置",
        validateParameters = {
    @Field(fieldName = "session", fieldType = FieldTypeEnum.CHAR1024, description = "session信息"),
    @Field(fieldName = "encryptType", fieldType = FieldTypeEnum.TYINT, description = "加密类型")
})
public class InquireMemberCardLevelListService implements Service {

    private Logger logger = LogFactory.getInstance().getLogger(InquireMemberCardLevelListService.class);
    ...
    public void execute() {
        ...
        List<MemberCardLevel> entityList = entityDAO.inquireByCondition(conditionList);
        if (entityList != null) {
