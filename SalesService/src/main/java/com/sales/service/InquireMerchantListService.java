 ...
 */
@ServiceConfig(
        act = ActionNames.inquireMerchantList,
        ...
        description = "查询Merchant配置",
        validateParameters = {
    @Field(fieldName = "session", fieldType = FieldTypeEnum.CHAR1024, description = "session信息"),
    @Field(fieldName = "encryptType", fieldType = FieldTypeEnum.TYINT, description = "加密类型")
})
public class InquireMerchantListService implements Service {

    private Logger logger = LogFactory.getInstance().getLogger(InquireMerchantListService.class);
    ...
    public void execute() {
        ...
        List<Merchant> entityList = entityDAO.inquireByCondition(conditionList);
        if (entityList != null) {
