 ...
 */
@ServiceConfig(
        act = ActionNames.inquirePayFlowList,
        ...
        description = "查询PayFlow配置",
        validateParameters = {
    @Field(fieldName = "session", fieldType = FieldTypeEnum.CHAR1024, description = "session信息"),
    @Field(fieldName = "encryptType", fieldType = FieldTypeEnum.TYINT, description = "加密类型")
})
public class InquirePayFlowListService implements Service {

    private Logger logger = LogFactory.getInstance().getLogger(InquirePayFlowListService.class);
    ...
    public void execute() {
        ...
        List<PayFlow> entityList = entityDAO.inquireByCondition(conditionList);
        if (entityList != null) {
