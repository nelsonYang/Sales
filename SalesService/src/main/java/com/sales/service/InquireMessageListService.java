 ...
 */
@ServiceConfig(
        act = ActionNames.inquireMessageList,
        ...
        description = "查询Message配置",
        validateParameters = {
    @Field(fieldName = "session", fieldType = FieldTypeEnum.CHAR1024, description = "session信息"),
    @Field(fieldName = "encryptType", fieldType = FieldTypeEnum.TYINT, description = "加密类型")
})
public class InquireMessageListService implements Service {

    private Logger logger = LogFactory.getInstance().getLogger(InquireMessageListService.class);
    ...
    public void execute() {
        ...
        List<Message> entityList = entityDAO.inquireByCondition(conditionList);
        if (entityList != null) {
