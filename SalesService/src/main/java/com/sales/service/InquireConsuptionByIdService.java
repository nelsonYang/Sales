 ...
 */
@ServiceConfig(
        act = ActionNames.inquireConsuptionById,
...
public class InquireConsuptionByIdService implements Service {
    
    private Logger logger = LogFactory.getInstance().getLogger(InquireConsuptionByIdService.class);
	...
	primaryKey.putKeyField("consuptionId",String.valueOf(consuptionId));
        ...
        EntityDao<Consuption> entityDAO = applicationContext.getEntityDAO(EntityNames.consuption);
          Consuption entity = entityDAO.inqurieByKey(primaryKey);
        if (entity != null) {
