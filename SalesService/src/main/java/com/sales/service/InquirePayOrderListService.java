 ...
 */
@ServiceConfig(
        act = ActionNames.inquirePayOrderList,
        ...
        description = "查询PayOrder配置",
        validateParameters = {
    @Field(fieldName = "session", fieldType = FieldTypeEnum.CHAR1024, description = "session信息"),
    @Field(fieldName = "encryptType", fieldType = FieldTypeEnum.TYINT, description = "加密类型")
})
public class InquirePayOrderListService implements Service {

    private Logger logger = LogFactory.getInstance().getLogger(InquirePayOrderListService.class);
    ...
    public void execute() {
        ...
        List<PayOrder> entityList = entityDAO.inquireByCondition(conditionList);
        if (entityList != null) {
