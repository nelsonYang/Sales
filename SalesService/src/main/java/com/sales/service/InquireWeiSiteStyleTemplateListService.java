 ...
 */
@ServiceConfig(
        act = ActionNames.inquireWeiSiteStyleTemplateList,
        ...
        description = "查询WeiSiteStyleTemplate配置",
        validateParameters = {
    @Field(fieldName = "session", fieldType = FieldTypeEnum.CHAR1024, description = "session信息"),
    @Field(fieldName = "encryptType", fieldType = FieldTypeEnum.TYINT, description = "加密类型")
})
public class InquireWeiSiteStyleTemplateListService implements Service {

    private Logger logger = LogFactory.getInstance().getLogger(InquireWeiSiteStyleTemplateListService.class);
    ...
    public void execute() {
        ...
        List<WeiSiteStyleTemplate> entityList = entityDAO.inquireByCondition(conditionList);
        if (entityList != null) {
