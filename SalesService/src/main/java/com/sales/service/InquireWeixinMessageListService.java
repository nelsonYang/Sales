 ...
 */
@ServiceConfig(
        act = ActionNames.inquireWeixinMessageList,
        ...
        description = "查询WeixinMessage配置",
        validateParameters = {
    @Field(fieldName = "session", fieldType = FieldTypeEnum.CHAR1024, description = "session信息"),
    @Field(fieldName = "encryptType", fieldType = FieldTypeEnum.TYINT, description = "加密类型")
})
public class InquireWeixinMessageListService implements Service {

    private Logger logger = LogFactory.getInstance().getLogger(InquireWeixinMessageListService.class);
    ...
    public void execute() {
        ...
        List<WeixinMessage> entityList = entityDAO.inquireByCondition(conditionList);
        if (entityList != null) {
