 ...
 */
@ServiceConfig(
        act = ActionNames.inquireAwardResultById,
...
public class InquireAwardResultByIdService implements Service {
    
    private Logger logger = LogFactory.getInstance().getLogger(InquireAwardResultByIdService.class);
	...
	primaryKey.putKeyField("awardResultId",String.valueOf(awardResultId));
        ...
        EntityDao<AwardResult> entityDAO = applicationContext.getEntityDAO(EntityNames.awardResult);
          AwardResult entity = entityDAO.inqurieByKey(primaryKey);
        if (entity != null) {
