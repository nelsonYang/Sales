 ...
 */
@ServiceConfig(
        act = ActionNames.inquireEventFlowList,
        ...
        description = "查询EventFlow配置",
        validateParameters = {
    @Field(fieldName = "session", fieldType = FieldTypeEnum.CHAR1024, description = "session信息"),
    @Field(fieldName = "encryptType", fieldType = FieldTypeEnum.TYINT, description = "加密类型")
})
public class InquireEventFlowListService implements Service {

    private Logger logger = LogFactory.getInstance().getLogger(InquireEventFlowListService.class);
    ...
    public void execute() {
        ...
        List<EventFlow> entityList = entityDAO.inquireByCondition(conditionList);
        if (entityList != null) {
