 ...
 */
@ServiceConfig(
        act = ActionNames.inquireCompanyList,
        ...
        description = "查询Company配置",
        validateParameters = {
    @Field(fieldName = "session", fieldType = FieldTypeEnum.CHAR1024, description = "session信息"),
    @Field(fieldName = "encryptType", fieldType = FieldTypeEnum.TYINT, description = "加密类型")
})
public class InquireCompanyListService implements Service {

    private Logger logger = LogFactory.getInstance().getLogger(InquireCompanyListService.class);
    ...
    public void execute() {
        ...
        List<Company> entityList = entityDAO.inquireByCondition(conditionList);
        if (entityList != null) {
