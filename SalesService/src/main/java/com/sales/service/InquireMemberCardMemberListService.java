 ...
 */
@ServiceConfig(
        act = ActionNames.inquireMemberCardMemberList,
        ...
        description = "查询MemberCardMember配置",
        validateParameters = {
    @Field(fieldName = "session", fieldType = FieldTypeEnum.CHAR1024, description = "session信息"),
    @Field(fieldName = "encryptType", fieldType = FieldTypeEnum.TYINT, description = "加密类型")
})
public class InquireMemberCardMemberListService implements Service {

    private Logger logger = LogFactory.getInstance().getLogger(InquireMemberCardMemberListService.class);
    ...
    public void execute() {
        ...
        List<MemberCardMember> entityList = entityDAO.inquireByCondition(conditionList);
        if (entityList != null) {
