 ...
 */
@ServiceConfig(
        act = ActionNames.inquireEventById,
...
public class InquireEventByIdService implements Service {
    
    private Logger logger = LogFactory.getInstance().getLogger(InquireEventByIdService.class);
	...
	primaryKey.putKeyField("eventId",String.valueOf(eventId));
        ...
        EntityDao<Event> entityDAO = applicationContext.getEntityDAO(EntityNames.event);
          Event entity = entityDAO.inqurieByKey(primaryKey);
        if (entity != null) {
