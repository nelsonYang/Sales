 ...
 */
@ServiceConfig(
        act = ActionNames.inquirePersonCardList,
        ...
        description = "查询PersonCard配置",
        validateParameters = {
    @Field(fieldName = "session", fieldType = FieldTypeEnum.CHAR1024, description = "session信息"),
    @Field(fieldName = "encryptType", fieldType = FieldTypeEnum.TYINT, description = "加密类型")
})
public class InquirePersonCardListService implements Service {

    private Logger logger = LogFactory.getInstance().getLogger(InquirePersonCardListService.class);
    ...
    public void execute() {
        ...
        List<PersonCard> entityList = entityDAO.inquireByCondition(conditionList);
        if (entityList != null) {
