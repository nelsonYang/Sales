 ...
 */
@ServiceConfig(
        act = ActionNames.inquireCouponList,
        ...
        description = "查询Coupon配置",
        validateParameters = {
    @Field(fieldName = "session", fieldType = FieldTypeEnum.CHAR1024, description = "session信息"),
    @Field(fieldName = "encryptType", fieldType = FieldTypeEnum.TYINT, description = "加密类型")
})
public class InquireCouponListService implements Service {

    private Logger logger = LogFactory.getInstance().getLogger(InquireCouponListService.class);
    ...
    public void execute() {
        ...
        List<Coupon> entityList = entityDAO.inquireByCondition(conditionList);
        if (entityList != null) {
