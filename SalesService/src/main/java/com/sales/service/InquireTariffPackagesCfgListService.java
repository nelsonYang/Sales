 ...
 */
@ServiceConfig(
        act = ActionNames.inquireTariffPackagesCfgList,
        ...
        description = "查询TariffPackagesCfg配置",
        validateParameters = {
    @Field(fieldName = "session", fieldType = FieldTypeEnum.CHAR1024, description = "session信息"),
    @Field(fieldName = "encryptType", fieldType = FieldTypeEnum.TYINT, description = "加密类型")
})
public class InquireTariffPackagesCfgListService implements Service {

    private Logger logger = LogFactory.getInstance().getLogger(InquireTariffPackagesCfgListService.class);
    ...
    public void execute() {
        ...
        List<TariffPackagesCfg> entityList = entityDAO.inquireByCondition(conditionList);
        if (entityList != null) {
