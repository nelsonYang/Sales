 ...
 */
@ServiceConfig(
        act = ActionNames.inquireWeiBarList,
        ...
        description = "查询WeiBar配置",
        validateParameters = {
    @Field(fieldName = "session", fieldType = FieldTypeEnum.CHAR1024, description = "session信息"),
    @Field(fieldName = "encryptType", fieldType = FieldTypeEnum.TYINT, description = "加密类型")
})
public class InquireWeiBarListService implements Service {

    private Logger logger = LogFactory.getInstance().getLogger(InquireWeiBarListService.class);
    ...
    public void execute() {
        ...
        List<WeiBar> entityList = entityDAO.inquireByCondition(conditionList);
        if (entityList != null) {
