 ...
 */
@ServiceConfig(
        act = ActionNames.inquireWeiSiteSytleTemplateConfigList,
        ...
        description = "查询WeiSiteSytleTemplateConfig配置",
        validateParameters = {
    @Field(fieldName = "session", fieldType = FieldTypeEnum.CHAR1024, description = "session信息"),
    @Field(fieldName = "encryptType", fieldType = FieldTypeEnum.TYINT, description = "加密类型")
})
public class InquireWeiSiteSytleTemplateConfigListService implements Service {

    private Logger logger = LogFactory.getInstance().getLogger(InquireWeiSiteSytleTemplateConfigListService.class);
    ...
    public void execute() {
        ...
        List<WeiSiteSytleTemplateConfig> entityList = entityDAO.inquireByCondition(conditionList);
        if (entityList != null) {
